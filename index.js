...
const updatedSpreadsheet = "assets/HISTÓRICO E CONTATO ALUNOS ATIVOS 18022021.ods";
const oldSpreadsheet = "assets/HISTÓRICO E CONTATO ALUNOS ATIVOS 08092020.ods"; // used for sheetsCompare only
...
const capStart = (str) => {
  ...
  for (let p = 0; p < split.length; p++) {
    split[p] =
      split[p].slice(0, 1).toUpperCase() + split[p].slice(1).toLowerCase();
  }
  return split.join(" ");
};
...
(async () => {
  ...
  const readSecacSpreadSheet = (name) => {
    ...
    for (let sheet of sheetNames) {
      ...
      let phone1Collumn = findCollumn(
        acs,
        "TELEFONE 1",
        findCollumn(acs, "TEL. FIXO", "K")
      );
      let phone2Collumn = findCollumn(
        acs,
        "TELEFONE 2",
        findCollumn(acs, "CELULAR", "L")
      );
      let phone3Collumn = findCollumn(
        acs,
        "CEL. RESPONSÁVEL",
        findCollumn(acs, "CEL. RESPONSÁVEL", "M")
      );
      ...
      for (let i = 3; i < 50; i++) {
        ...
        if (nameCell && nameCell.v) {
          // if this line has text at the name collumn
          ...
          if (
            ...
            (nameCell &&
              !status &&
              (sheet === "Mat 2020" || sheet === "Eng Elet 2020"))
          ) {
            // Temp fix due incomplete tabs at external .ods file
    ...
    console.log(
      students.length +
        " active student records found at records spreadsheet " +
        name +
        "\n"
    );
  ...
  const res = readSecacSpreadSheet(updatedSpreadsheet);
  ...
  if (mode === "cardsCompletion") {
    ...
    const idCardsAll = XLSX.readFile(
      "assets/Controle fotos carteiras estudantis _ carteirinhas de estudante  2020.ods"
    );
    ...
    for (let i = 4; i < 600; i++) {
      ...
      if (nameCell && nameCell.v && nameCell.v.includes("-")) {
        const name = utils
          .accentFold(nameCell.v.toLowerCase().split("-")[0])
          .trim();
        const tab = tagToTab(nameCell.v.split("-")[1]);
        const filteredStudents = tab
          ? students.filter((stu) => stu.sheet === tab)
          : students;
        ...
        if (!rec) {
          // If no match was found using exact name match at expected tab, tries using all other tabs data
        ...
        if (!rec) {
          // If no match was found for this student
          ...
          const findSimilarName = (fName, fStudents) => {
            const fSimilarName = stringSimilarToAny(
              fName,
              fStudents.map((cur) => cur.name)
            );
            let fSimilarCompleted = null;
            for (let s = 0; s < fStudents.length; s++) {
              if (fStudents[s].name === fSimilarName) {
                fSimilarCompleted =
                  capStart(fStudents[s].nameUntreated) +
                  '" - ' +
                  fStudents[s].sheet;
              }
            }
    ...
    notFoundNames = notFoundNames.map((cur, i) => {
      ...
      if (didYouMean[i]) {
        ...
        // look in inactive students
        const inactiveRecord = studenRecordOf(name, notActiveStudents);
        if (inactiveRecord) {
          name =
            name +
            ". Status: " +
            inactiveRecord.status +
            " (" +
            inactiveRecord.sheet +
            ")";
        }
      }
      return name;
    ...
    if (notFoundCount) {
      // Prints not found cases to console
      console.log();
    ...
    const regToFindCount = valuesToFind.filter((cur) => cur === "register")
      .length;
    const birthToFindCount = valuesToFind.filter((cur) => cur === "birth")
      .length;

    const valuesFoundCount = valuesFound.length;
    const docFoundCount = valuesFound.filter((cur) => cur === "doc").length;
    const regFoundCount = valuesFound.filter((cur) => cur === "register")
      .length;
    ...
    console.log(
      "\n" +
        studentsIdsToPrintCount +
        " student records to print id cards, " +
        foundCount +
        " students found (" +
        ((100 * foundCount) / studentsIdsToPrintCount).toFixed(1) +
        " %), " +
        notFoundCount +
        " not found"
    );
    console.log(
      valuesToFindCount +
        ...
        valuesFoundCount +
        " (" +
        ((100 * valuesFoundCount) / valuesToFindCount).toFixed(1) +
        " %) values found and completed\n"
    );
    ...
    XLSX.writeFile(
      idCardsAll,
      "assets/Controle fotos carteiras estudantis _ carteirinhas de estudante __ COMPLETED.ods"
    );
    ...
    const model = XLSX.readFile("assets/Student default form.xlsx", {
      cellStyles: true,
      type: "file",
      cellHTML: true,
      dense: true,
    ...
    for (const sheet of selectedSheets) {
      ...
      for (const student of classStudents) {
        ...
        dataSheet["I2"] = {
          v: student.entranceAt,
          t: "s",
          w: student.entranceAt,
        }; // entry time
        dataSheet["J2"] = {
          v: student.entranceKind,
          t: "s",
          w: student.entranceKind,
        }; // entry type

        newSheet.Sheets["Dados"] = dataSheet;
        XLSX.writeFile(
          newSheet,
          "outputs/" + sheet + "/" + student.nameUntreated + ".xlsx",
          {
            cellStyles: true,
            type: "file",
            cellHTML: true,
      ...
      console.log(
        "Class " + sheet + ": " + classStudents.length + " spreadsheets saved"
      );
    }
  } else {
    // sheetsCompare

    const old = readSecacSpreadSheet(oldSpreadsheet);

    // finds students that were active at old sheet, but aren't active now
    let nowInactiveList = [];
    for (const inactiveStudent of notActiveStudents) {
      if (
        inactiveStudent.sheet !== "Quantitativo" &&
        !old.notActiveStudents.some(
          (student) => student.doc === inactiveStudent.doc
        )
      ) {
        // he/she wasn't inactive before
    ...
    for (const student of students) {
      const match = old.students.find(
        (cur) =>
          (cur.doc && cur.doc === student.doc && cur.sheet === student.sheet) || // find by doc
          (!cur.doc &&
            cur.sheet === student.sheet &&
            cur.nameUntreated === student.nameUntreated)
      ); // if doc is not available
      if (match) {
        let difs = "";
        ...
        const changes = fieldsToCheck
          .map((field) => {
            if (student[field] !== match[field]) {
              if (field === "name") {
                //  && student[field].toLowerCase() === match[field].toLowerCase()
        ...
        // check phones
        const oldPArr = [match.phone1, match.phone2, match.phone3].filter(
          (el) => !!el && countNumbers(el) > 7
        );
        const oldP = oldPArr.sort((a, b) => a - b).join(", ");
        const newPArr = [student.phone1, student.phone2, student.phone3].filter(
          (el) => !!el && countNumbers(el) > 7
        );
        ...
        if (oldP !== newP) {
          ...
          const isRemovedArr = oldPArr.filter(
            (newps) => !newPArr.includes(newps)
          );
        ...
        if (difs) {
          ...
          changesArr.push(
            capStart(student.name) + " (" + student.sheet + "): " + difs
          );
        }
      } else {
        noMatchCount++;
